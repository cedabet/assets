...
.active-playlist-item {
  background-color: #f1f1f1; /* Veya istediğiniz başka bir renk */
}
.playlist-item img {
  width: 70%;
  aspect-ratio: 1/1;
...
const swiper = new Swiper("#custom-swiper", {
  ...
  grabCursor: true,
  speed: 700,
  initialSlide: 2, // Başlangıç slide'ı
});

playlistItems.forEach((item, index) => {
  // Playlist item'ına tıklanma olayını ekleyelim
  item.addEventListener("click", () => {
    // Swiper'ı tıklanan playlist item'ına göre güncelle
    swiper.slideTo(index); // Hangi item tıklanırsa, Swiper o slide'a gider
  });
});

swiper.on("slideChange", () => {
  const newIndex = swiper.realIndex;
  updatePlaylistHighlight(newIndex);
});

function updatePlaylistHighlight(index) {
  playlistItems.forEach((item, i) => {
    if (i === index) {
      item.classList.add("active-playlist-item");  // Aktif class ekle
    } else {
      item.classList.remove("active-playlist-item");  // Diğerlerinden kaldır
    }
  });
}
