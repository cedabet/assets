...
function addMenuElement() {
    ...
    const bigWinsWrapper = document.getElementById('big-wins-wrapper'); // 'big-wins-wrapper' id'sine sahip section'ı alıyoruz

    if (!sidebarNav || !bigWinsWrapper) {
        return;
    }
    ...
    // 'big-wins-wrapper' içindeki use href'i al
    const useElement = document.querySelector('#big-wins-wrapper svg.svg-icon use');

    // use elementinin href değerini al
    const hrefValue = useElement ? (useElement.getAttribute('href') || useElement.getAttribute('xlink:href')) : '';

    // hrefValue'yi konsola yazdır
    console.log('Href Value:', hrefValue);

    // İkinci li: Latest Big Wins (Table Games'in hemen altına)
    const newLi2 = document.createElement('li');
    newLi2.id = 'custom-item-latest-big-wins';
    ...
    use.setAttribute('href', hrefValue);  // Burada dinamik olarak aldığımız href değerini kullanıyoruz

    svgIcon.appendChild(use);
    newAnchor2.appendChild(svgIcon);
    newAnchor2.appendChild(document.createTextNode(' Latest Big Wins'));

    const span2 = document.createElement('span');
    span2.textContent = 'New';
    newAnchor2.appendChild(span2);

    newLi2.appendChild(newAnchor2);

    // Table Games li'sinin hemen altına ekle
    sidebarNav.insertBefore(newLi2, newLi.nextSibling);
}
