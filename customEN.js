...
function latestBigWins() {
    ...
    const featureSectionHTML = `
        <section class="feature-section">
            ...
            <div class="game-container">
                <a class="game-item" href="https://ornek-link.com/sweet-bonanza">
                    <img src="https://picsum.photos/150/100" alt="Sweet Bonanza">
                    <p>Sweet Bonanza</p>
                </a>
                <a class="game-item" href="https://ornek-link.com/book-of-dead">
                    <img src="https://picsum.photos/150/100" alt="Book of Dead">
                    <p>Book of Dead</p>
                </a>
                <a class="game-item" href="https://ornek-link.com/starburst">
                    <img src="https://picsum.photos/150/100" alt="Starburst">
                    <p>Starburst</p>
                </a>
                <a class="game-item" href="https://ornek-link.com/gonzos-quest">
                    <img src="https://picsum.photos/150/100" alt="Gonzo's Quest">
                    <p>Gonzo’s Quest</p>
                </a>
                <a class="game-item" href="https://ornek-link.com/mega-moolah">
                    <img src="https://picsum.photos/150/100" alt="Mega Moolah">
                    <p>Mega Moolah</p>
                </a>
                <a class="game-item" href="https://ornek-link.com/dead-or-alive">
                    <img src="https://picsum.photos/150/100" alt="Dead or Alive">
                    <p>Dead or Alive</p>
                </a>
            </div>
        </section>
    ...
    const winSectionHTML = `
        <h2>Son Büyük Kazançlar</h2>
    ...
    const games = [
        'Sweet Bonanza', 'Wolf Gold', 'The Dog House', 'Great Rhino', 'John Hunter and the Tomb of the Scarab Queen',
        'Mustang Gold', 'Gates of Olympus', 'Hot to Burn', 'Sweet Bonanza Xmas', 'Fruit Party', 'Madame Destiny',
        'Jungle Gorilla', '7 Monkeys', 'Buffalo King', 'Pirate Gold', 'Power of Thor Megaways', 'Money Train 2',
        'Release the Kraken', 'Queen of Gods', 'The Hand of Midas', 'Shining Crown', 'Aztec Treasure', 'Burning Hot',
        'Diamond Cash', 'Fantastic 7', '20 Super Hot', '5 Dazzling Hot', '40 Super Hot', 'Lords of Luck', 
        'Sizzling Hot Deluxe', '20 Burning Hot', 'Extra Stars', 'Ultra Hot Deluxe', 'Banana Splash', 'Olympic Games',
        'Pharaoh’s Gold', 'Lucky Lady’s Charm', 'Hot Wheels', 'Secret Forest', 'Golden Ark', 'Dead or Alive 2',
        'Jungle Spirit', 'Bonanza', 'Legacy of Egypt', 'Book of Dead', 'Reactoonz', 'Fishin’ Frenzy', 'Immortal Romance',
        'Big Bass Bonanza', 'Rise of Olympus', 'Dragon’s Fire', 'Buffalo Rising Megaways', 'White Rabbit', 'Chilli Heat',
        'Razor Shark', 'Gonzo’s Quest Megaways', 'Hypernova Megaways', 'Moon Princess', 'Fire Joker', 'Wild Toro'
    ];

    function generateRandomName() {
        const firstName = alphabet.charAt(Math.floor(Math.random() * alphabet.length)) + '*'.repeat(Math.floor(Math.random() * 6) + 3);
        const lastName = alphabet.charAt(Math.floor(Math.random() * alphabet.length)) + '*'.repeat(Math.floor(Math.random() * 6) + 3);
        return `${firstName} ${lastName}`;
    }

    function getRandomWin() {
        const r = Math.random();
        if (r < 0.15) return `${Math.floor(Math.random() * 40000) + 10000}₺`;
        else return `${Math.floor(Math.random() * 9600) + 400}₺`;
    }

    function getRandomInterval() {
        const base = Math.random();
        if (base < 0.4) return Math.floor(Math.random() * 1000) + 1000;     // 1-2 sn
        else if (base < 0.65) return Math.floor(Math.random() * 2000) + 2000; // 2-4 sn
        else if (base < 0.8) return Math.floor(Math.random() * 2000) + 4000;  // 4-6 sn
        else if (base < 0.9) return Math.floor(Math.random() * 2000) + 6000;  // 6-8 sn
        else return Math.floor(Math.random() * 2000) + 8000;                  // 8-10 sn
    }

    // FadeOut animasyonu 1 saniye sürüyor, Promise ile bekleyelim
    ...
    async function addWinner() {
        ...
        winItem.innerHTML = `
            <p><strong>Oyuncumuz ${name}</strong></p>
            <p><strong>${game}</strong> oyunundan <strong>${winAmount}</strong> kazandı.</p>
            <p>Tebrik ederiz, bol kazançlar!</p>
        `;

        if (winList.children.length >= 3) {
            const firstItem = winList.children[0];
            await fadeOutAndRemove(firstItem);  // FadeOut tamamlanana kadar bekle
    ...
    // Kazananları belirli aralıklarla eklemek
    async function scheduleNextWinner() {
        while (true) {
